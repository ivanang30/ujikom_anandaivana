...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validasi NISN tidak boleh kosong
    elseif (empty($nisn)) {
        ...
        if (mysqli_query($koneksi, $query)) {
            // Auto-generate 12 pembayaran untuk setiap siswa baru
            $bulan_names = array('Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 
                                'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember');
            
            for ($i = 1; $i <= 12; $i++) {
                $id_pembayaran = generateID('PB', 'tb_pembayaran', 'id_pembayaran');
                $bulan = str_pad($i, 2, '0', STR_PAD_LEFT);
                $tgl_bayar = "2025-$bulan-01";
                $batas_pembayaran = "2025-$bulan-10";
                
                $query_pembayaran = "INSERT INTO tb_pembayaran 
                                    (id_pembayaran, status, nisn, tgl_bayar, tgl_terakhir_bayar, 
                                     batas_pembayaran, jumlah_bulan, id_spp, nominal_bayar, jumlah_bayar, kembalian)
                                    VALUES ('$id_pembayaran', 'Belum Lunas', '$nisn', '$tgl_bayar', NULL, 
                                            '$batas_pembayaran', '1', '$id_spp', '0', '0', '0')";
                
                mysqli_query($koneksi, $query_pembayaran);
            }
            
            redirect('index.php', 'Data siswa berhasil ditambahkan! 12 bulan pembayaran otomatis dibuat.');
        } else {
...
<body>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control" id="nis" name="nis" value="<?php echo $nis_otomatis; ?>" required>
                                            <button class="btn btn-outline-secondary" type="button" id="btnGenerateNIS" onclick="generateNISBaru()">
                                                <i class="fas fa-sync"></i> Generate
                                            </button>
                                        </div>
                                        <small class="text-muted">NIS otomatis terisi. Klik Generate untuk NIS baru atau ubah manual.</small>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        function generateNISBaru() {
            // Generate NIS baru (increment dari NIS terakhir)
            const currentNIS = document.getElementById('nis').value;
            const nextNIS = String(parseInt(currentNIS) + 1).padStart(8, '0');
            document.getElementById('nis').value = nextNIS;
        }
    </script>
</body>
</html>
